...
import { getLatestRewardPayload, RewardPayload, getRewardHistory } from "../data-fetcher"
...
export default function Index(props: RewardsProps) {
  ...
  const [userRewardPayloads, setUserRewardPayloads] =
    useState<RewardPayload[]>([]);
  ...
  const query = async () => {
    ...
    try {
      ...
      const minCycle = maxCycle - 50;
      setUserStartCycle(minCycle)
      setUserEndCycle(maxCycle+1)
      await getRewardHistory(userAddress, maxCycle, minCycle, (payload: RewardPayload) => {
  ...
  const latestCycleRewards = ethers.utils.formatEther(latestCyclePayload?.summary.cycleTotal || 0);
  const lifetimeRewards = ethers.utils.formatEther(latestCyclePayload?.payload.amount || 0);
  ...
  const getRewardsByToken = (): RewardsByTokenDetails => {
    ...
    userRewardPayloads.forEach(payload => {
      payload.summary.breakdown.forEach(detail => {
        ...
        if (ld) {
          currentDetail.liquidityDirector = currentDetail.liquidityDirector.add(detail.amount);
          ldTotal = ldTotal.add(detail.amount);
        } else {
          currentDetail.liquidityProvider = currentDetail.liquidityProvider.add(detail.amount);
  ...
  const getRewardsByCycleAndToken = (): RewardsByCycleAndToken[] => {
    ...
    userRewardPayloads.forEach(payload => {
      payload.summary.breakdown.forEach(detail => {
        ...
        if (ld) {
          cycleTokens[cycleTokenKey].liquidityDirector = BigNumber.from(detail.amount);
        } else {
          cycleTokens[cycleTokenKey].liquidityProvider = BigNumber.from(detail.amount);
        }
      });
    })
...
function getTokenAndisLdFromDescription(description: string): [string, boolean] {
